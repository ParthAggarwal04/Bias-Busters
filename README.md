...
## Project Structure
    ...
    │
    ├── sample_data.csv
    ├── sample_data_corrected.csv
    ├── adjusted_data.csv
    │
    └── tests/
    ├── test_script.py
    ├── test_bias_adjustment.py
    └── test_bias_correction.py

### Main Application Files

- **app.py**  
  Flask backend that handles dataset uploads, bias analysis, mitigation, and downloading corrected datasets.

- **streamlit_app.py**  
  Streamlit interface for running the Bias Buster tool in a simplified web UI and deploying on Streamlit Cloud.

### Bias Detection & Mitigation Logic

`bias/` contains the core fairness analysis modules.

- **metrics.py**  
  Implements fairness evaluation functions such as:
  - Disparate Impact
  - Group prediction rates
  - Representation imbalance

- **mitigate.py**  
  Implements bias mitigation techniques including:
  - Reweighing
  - Dataset resampling
...
- **templates/index.html**  
  Main UI for the Flask application.

- **static/app.js**  
  JavaScript logic that connects the frontend interface with backend APIs.
...
- **outputs/**  
  Stores corrected or bias-mitigated datasets generated by the system.

### Sample Data

- **sample_data.csv**  
  Example dataset used for testing bias detection.

- **sample_data_corrected.csv**  
  Example dataset after bias mitigation.

- **adjusted_data.csv**  
  Output dataset generated after applying fairness adjustments.

### Testing

The `tests/` folder contains scripts used to verify bias detection and mitigation logic.

- **test_script.py**  
  Tests mitigation functions.

- **test_bias_adjustment.py**  
  Tests API flow and dataset correction.

- **test_bias_correction.py**  
  Validates fairness correction in hiring datasets.

### Deployment Files

- **requirements.txt** – Python dependencies  
- **Procfile** – Deployment configuration for platforms like Heroku/Render  
